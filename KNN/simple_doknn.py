...
def doKNN(X_train, X_test, y_train, y_test,params={}):
    ...
    recall_score = []
    f1_score = []
    if not('n_neighbors' in params.keys()):
    ...
    return {'precision': max(precision_score), 'accuracy': max(scores_list), 'auc': max(auc_score), 'recall': max(recall_score), 'f1_measure': max(f1_score), 'algorithm' : knn,'balanced_accuracy':metrics.balanced_accuracy_score(y_test,y_pred)}